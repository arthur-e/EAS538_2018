...
> Arthur, please check if my understanding make sense.
...
contable <- bechdel %>%
  ddply(~decade_code, summarise,
        yes = length(which(binary=="PASS")),
        no = length(which(binary=="FAIL")))
chisq.test(contable[,2:3])
```

This indicates that the reason for the decreased p-value of the `binary` variable is because the `binary` and `decade_code` variables are not independent. 
