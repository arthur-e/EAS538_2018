...
You can also force the linear regression model to *not* to include a baseline/ reference level with including `-1` in the `lm()`. Let's try it!

```{r, lm ANCOVA 3_1, eval = FALSE}
linmod3 <- lm(juveniles ~ -1 + temperature + rainfall + humanpop + country + sand, data = dataset)
summary(linmod3)
```
...
```{r}
linmod <- lm(juveniles ~ sand, data = dataset)
summary(linmod)
```
