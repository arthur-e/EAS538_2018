...
The two variables are negatively correlated, which means that as `nox` increases, `medv` decreases (or vice versa). We consider the correlation is neither very strong nor very weak because the correlation coefficient `r cor(house$nox,house$medv)` is not very high. 
...
1. Identically distributed (homoscedastic) residuals  
...
plot(residuals(linmod) ~ fitted(linmod),
...
From both the qqplot and the shapiro test, we see that the residuals are not normally distributed. You can get one point extra credit if you try to transforme the data and describe whether it improved/made worse normality and homoscedasticity.

3. Independence in the residuals  
Since we did not explicitely talk about the independence in the residuals, we did not grade this part. However, given the nature of this data, we woudl expect there is spatial autocorrelation among these housing data. 
...
From the summary table and the plot we see that nitric oxides concentration (parts per 10 million; `nox`) negatively affects the median value of owner-occupied homes in $1000 (`medv`). With 1 ppm increase of nitric oxides concentration, median value of owner-occupied homes should decrease `r round(-summary(linmod)$coefficients[2,1]*1000, 1)`. If the nitric oxides concentration is 0, the median value of owner-occupied homes is expected to be `r round(-summary(linmod)$coefficients[1,1]*1000, 1)`. 

> e. Discuss the fit of your model and whether you think it is a good or bad fit. Why?  

We see consider linear model is not a good fit to the data because  
1. The adjusted $R^2$ is `r summary(linmod)$adj.r.squared`, which pretty low. Only 10 % of the variance of the median value of owner-occupied homes.  
2. The normality of the residual assumption is not met. 