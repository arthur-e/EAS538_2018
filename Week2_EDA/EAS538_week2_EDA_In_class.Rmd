...
First, slice the `rent.US` data frame so it only contains the first 10 rows.
Because `rent.US` is already orderd by `Population.Rank`, this corresponds to the top 10 U.S. cities by population.

```{r}
rent.US.top10 <- rent.US[1:10,]
```

1. Now, **using the `rent.US.top10` data frame**, pick any 2 or more variables/ columns with different data types. (Recall that you can access a variable in a data frame by using the `$` symbol.) Create a list containing multiple types of data (vectors from `rent.US.top10`) with `list()`.
2. Pick another variable from `rent.US.top10`. Create a matrix with `matrix()` to store that variable. The `nrow` and `ncol` arguments are optional, but what should they be set to if you want to create a 1-row, 10-column matrix (for all 10 values in your chosen variable)?
...
**Hint:** In the R Console, you can type `View(df)`, where `df` is the name of any data frame, to look at the contents of a data frame in a new tab.

**Hint:** We can line up two vectors together with `cbind()` in order to, for example, assign numbered rankings to a list of cities:
```{r}
integers <- 1:nrow(rent.MI)
cbind(integers, rent.MI$City)
```
...
**Hint:** Earlier, we saw an R function that lets you subset the rows of a data frame.

__Optional Challenges__

* How can you set the x-axis or y-axis label of a histogram? The title of the plot? Is there more than one way to do this? Read `help(hist)` but also try to find the answer using a search engine. Try customizing your histogram's title and labels so that it is easier to understand at a glance.
